...
export default function AddJobListing() {
  ...
  const [standardRequirements, setStandardRequirements] = useState({
    resume: { label: 'Resume / CV', enabled: true, required: true },
    coverLetter: { label: 'Cover Letter', enabled: true, required: false },
    experience: { label: 'Work Experience', enabled: true, required: true },
    education: { label: 'Education Details', enabled: true, required: true },
    skills: { label: 'Skills', enabled: true, required: true },
    portfolioWebsite: { label: 'Personal Website / Portfolio', enabled: true, required: false },
    linkedin: { label: 'LinkedIn Profile', enabled: true, required: false },
    github: { label: 'GitHub Profile', enabled: true, required: false }
  });
  ...
  // Handle standard fields toggle
  const handleStandardFieldChange = (key, field) => {
    setStandardRequirements(prev => ({
      ...prev,
      [key]: {
        ...prev[key],
        [field]: !prev[key][field],
        // If disabling the field, also remove the 'required' check
        ...(field === 'enabled' && prev[key].enabled ? { required: false } : {})
      }
    }));
  };
  ...
  const handlePostJob = () => {
    // This payload contains everything structured for your backend
    const jobPayload = {
        jobDetails: formData,
      ...
      <div className="mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* Customize Application Form Section - Now Standard Information Only */}
            <div className="bg-[#F0ECE3] rounded-2xl shadow-sm border border-slate-200/60 p-6">
              <div className="mb-6">
                <h2 className="text-xl font-bold text-slate-900 mb-2">Customize Application Form</h2>
                <p className="text-sm text-slate-500">Select standard information to collect from applicants.</p>
              </div>

              {/* Standard Requirements */}
              <div>
                <h3 className="text-sm font-bold text-slate-900 mb-3 uppercase tracking-wider">Standard Information</h3>
                <div className="grid grid-cols-1 gap-3">
                  {Object.entries(standardRequirements).map(([key, value]) => (
                    <div key={key} className="flex flex-wrap items-center justify-between bg-[#F6F2E8] border border-slate-200 p-3.5 rounded-xl transition-all">
                      <span className="text-sm font-semibold text-slate-800">{value.label}</span>
                      ...
                      <div className="flex items-center gap-4">
                        <label className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer hover:text-slate-900">
                          <input 
                            ...
                            onChange={() => handleStandardFieldChange(key, 'enabled')}
                            className="w-4 h-4 text-teal-600 rounded border-slate-300 focus:ring-teal-500 focus:ring-2" 
                          />
                          Ask
                        </label>
                        <label className={`flex items-center gap-2 text-sm cursor-pointer ${value.enabled ? 'text-slate-600 hover:text-slate-900' : 'text-slate-400'}`}>
                          <input 
                            type="checkbox" 
                            checked={value.required} 
                            disabled={!value.enabled}
                            onChange={() => handleStandardFieldChange(key, 'required')}
                            className="w-4 h-4 text-teal-600 rounded border-slate-300 focus:ring-teal-500 focus:ring-2 disabled:opacity-50" 
                          />
                          Required
                        </label>
                      </div>
